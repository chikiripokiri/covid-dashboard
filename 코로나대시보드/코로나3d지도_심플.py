...
# Using Unsmoothed Resolution (500x600) as requested
...
def get_largest_polygon(geometry):
    ...
    if geometry['type'] == 'Polygon':
        ...
        # coordinates is a list of Polygons. Each Polygon is a list of Rings.
        # We want the Polygon with the longest exterior ring (index 0).
        polys = geometry['coordinates']
        if not polys:
            return geometry
        
        # Find max by length of exterior ring
...
for feature in sk_geojson['features']:
    ...
    # Apply Shapely Simplification to flatten jagged lines
    # Tolerance 0.05 is roughly 5km. Adjust for "flatness".
    shapely_geom = shape(largest_geometry)
    simplified_shapely = shapely_geom.simplify(0.05, preserve_topology=False)
    simplified_geometry = mapping(simplified_shapely)
        
    # Rasterize
    shapes = [(simplified_geometry, 1)]
...
# Create a mask of valid pixels (where we have data)
valid_mask = ~np.isnan(elevation)

# Dilate the valid mask to find the "Shoreline" pixels (just outside valid regions)
# We dilate by 1 pixel structure
shoreline_mask = binary_dilation(valid_mask, iterations=1) & ~valid_mask

# Set the shoreline pixels to 0 (Sea Level Base)
# This forces the Surface plot to draw a vertical wall from Level -> 0
...
fig = go.Figure(data=[go.Surface(
    ...
    cmin=0,
    cmax=18, # Use fixed max level for consistent coloring
...
fig.update_layout(
    title=f'COVID-19 3D Stepped Map (Levels 1-18) - {latest_date}',
    scene={
        "xaxis": {"visible": False}, 
        "yaxis": {"visible": False},
        "zaxis": {"title": "Level (1-18)", "visible": True},
        "aspectmode": "manual",
        # Adjust Z aspect ratio. Since Z is 1-18 and X is ~6, ratio 0.5 makes Z visual ~3.
        # This is reasonably tall.
...
output_file = "korea_covid_surface.html"
